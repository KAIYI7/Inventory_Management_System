...
class StockApiController extends Controller
{
    ...
    {
        $q = \App\Models\Stock::query()->with(['product:id,name,selling_price,is_active','branch:id,name']);

        // Exclude inactive products unless explicitly requested
        if (!$r->boolean('include_inactive_products')) {
            $q->whereHas('product', fn($qq) => $qq->where('is_active', true));
        }
